""" Include javascript templates inside <script type="text/x-tmpl">
syn region  JSTmplBlock		keepend matchgroup=JSTmplDelimiter start=+<%[=^#]\?+ end=+%>+  contains=@htmlJavaScript,htmlCssStyleComment,htmlScriptTag,@htmlPreproc contained
syn region  JSTmpl 		keepend matchgroup=JSTmplTag start=+<script [^>]*type *=[^>]*x-tmpl[^>]*>+ end=+</script>+ contains=@htmlTop,htmlScriptTag,@htmlPreproc,JSTmplBlock
